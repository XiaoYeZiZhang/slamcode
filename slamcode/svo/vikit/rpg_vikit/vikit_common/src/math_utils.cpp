...
// 三角化  R t  当前帧相机坐标系下坐标    参考帧相机坐标系下的坐标
// 返回的是像素点在当前帧坐标系下的三维坐标???????返回的深度其实是在两帧中深度的平均
// ref:  https://blog.csdn.net/yg838457845/article/details/81293286
Vector3d
...
{
  // project2d xyz-> x/z,y/z
  // 将三维坐标转换为二维坐标 做差值(X/Z,Y/Z)
  // X/Z Y/Z   -   X/Z Y/Z
  Vector2d e = project2d(f1) - project2d(f2);
  return error_multiplier2 * e.norm();
}

// feature1是当前帧的点  是单位球坐标
//  feature2是参考帧的点  是单位球坐标
double
               ...
               vector<int>& outliers)
{
  ...
  // 遍历所有的匹配角点  在相机坐标系下的三维坐标
  for(size_t j=0; j<features1.size(); ++j)
  {
    // 三角测量 得到像素点在当前帧坐标系下的三维坐标
