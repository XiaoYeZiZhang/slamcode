...
void vk::NLLSSolver<D, T>::optimizeGaussNewton(ModelType& model)
{
  ...
  for (iter_ = 0; iter_<n_iter_; ++iter_)
  {
    ...
    // 对于特征点周围的4*4图像块，得到每个像素的Ax=b的A和b
    // 返回平均残差 并得到Ax=b 中的A和b sparse_img_align.cpp中
    // 在迭代过程中 model位姿不断发生变化  Ax=b不断求解x  变化model 循环..
    ...
    // solve the linear system  求解Ax = b
    // solve可以求解出T’ 也就是相对位姿的扰动
    if(!solve())
    {
