...
// 当前帧在最优搜索层的图像   patch_with_border  patch_  10   当前帧对应尺度下的像素坐标  no_simd初始化为false(在feature_alignment.h中)
/*
...
//    float new_chi2 = 0.0;
    ...
    for(int y=0; y<patch_size_; ++y)
    {
    	// 当前帧以地图点投影的像素坐标为主的8*8图块
      uint8_t* it = (uint8_t*) cur_img.data + (v_r+y-halfpatch_size_)*cur_step + u_r-halfpatch_size_;
      for(int x=0; x<patch_size_; ++x, ++it, ++it_ref, ++it_ref_dx, ++it_ref_dy)
      {
      	// search_pixel：当前帧
        float search_pixel = wTL*it[0] + wTR*it[1] + wBL*it[cur_step] + wBR*it[cur_step+1];
        // 残差  it_ref是对应的参考帧8*8的像素块坐标
        float res = search_pixel - *it_ref + mean_diff;
        // 残差*参考帧的梯度
...
//         << "\t new chi2 = " << new_chi2 << endl;
  ...
  // 将更新后的当前帧的像素坐标 写入cur_px_estimate
  cur_px_estimate << u, v;
  return converged;
}
    ...
    Vector2d& cur_px_estimate)
{
  ...
  // Compute pixel location in new image:
  // cur_px_estimate在当前帧 对应尺度下的像素坐标
  float u = cur_px_estimate.x();
  float v = cur_px_estimate.y();
