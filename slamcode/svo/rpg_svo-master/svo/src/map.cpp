    ...
    std::list< std::pair<FramePtr,double> >& close_kfs) const
{
  ...
  for(auto kf : keyframes_)
  {
    ...
    for(auto keypoint : kf->key_pts_)
    {
      ...
      // 如果关键角点能在当前帧中投影，将相应关键帧    当前帧与相应关键帧的距离存放在close_fks中
...
void MapPointCandidates::addCandidatePointToFrame(FramePtr frame)
{
  ...
  // 遍历所有收敛的地图点
  // candidates_为PointCandidate类型的数组  PointCandidate为point  feature类型
  while(it != candidates_.end())
  {
  	// 如果深度收敛的地图点能够与当前帧的角点相对应，因为是收敛的地图点，直接将其作为特征加入到当前帧，并将其从candidate_中
  	// ???如果能被成为地图点的obs_，那不应该已经是当前帧的特征点么??????
